...
### 9. 🔐 会话级商业授权门禁、单窗口 1:1 店铺互斥与标准 6 步 SOP (Mode B 商业版)
- **标准 6 步极速开通上架 SOP**：
  1. **第一步【获取会话ID】**：在新窗口发送 `获取会话ID` 获取当前窗口专属 Conversation ID；
  2. **第二步【支付宝支付】**：支付宝扫码支付 600 元（¥600/店铺/窗口，单店 1:1 独立互斥隔离）；
  3. **第三步【获取授权码】**：支付成功后自动或由管理员签发专属防伪授权码 (License Key)；
  4. **第四步【激活当前窗口】**：在当前窗口发送 `激活授权 <你的商业授权码>`；
  5. **第五步【绑定目标店铺】**：发送 `绑定店铺 店铺简称：... API令牌：... 仓库ID：...`，完成 1:1 互斥锁定；
  6. **第六步【极速智能上架】**：直接发送 Ozon SKU 列表或 `.txt` 文件，全自动全要素搬家上架！
...
### 0. 🤖 Antigravity 智能体技能标准 6 步上架 SOP (推荐)

如果您在 Google Antigravity / Claude Code / Codex 等 AI 对话环境中使用本技能，安装本技能后，只需按以下 6 步即可完成专属授权与全自动上架：

```text
第1步【获取会话ID】：在新聊天窗口发送「获取会话ID」，获取当前窗口专属 Conversation ID。
第2步【支付宝支付】：支付宝扫码支付 600 元人民币（¥600/店铺/窗口，单店 1:1 独立互斥隔离）。
第3步【获取授权码】：支付成功后自动或由管理员签发专属防伪授权码 (License Key)。
第4步【激活当前窗口】：在当前窗口发送「激活授权 <你的商业授权码>」。
第5步【绑定目标店铺】：发送「绑定店铺 店铺简称：... API令牌：... 仓库ID：...」，完成 1:1 互斥锁定。
第6步【极速智能上架】：直接发送 Ozon SKU 列表或 .txt 文本路径，全自动全要素搬家上架！
```
