...
### 9. 🔐 会话级商业授权门禁、单窗口 1:1 店铺互斥与标准 5 步 SOP (Mode B 商业版)
- **标准 5 步极速开通上架 SOP**：
  1. **第一步【获取会话ID与支付】**：在新窗口发送 `获取会话ID` 获取当前窗口专属 Conversation ID，并在收银台支付宝扫码支付 600 元（¥600/店铺，一店一码 1:1 独立互斥隔离）；
  2. **第二步【自动获取授权码】**：支付成功后系统秒级自动签发专属防伪商业授权码 (License Key)；
  3. **第三步【激活当前窗口】**：在当前窗口发送 `激活授权 <你的商业授权码>`；
  4. **第四步【绑定目标店铺】**：发送 `绑定店铺 店铺简称：... API令牌：... 仓库ID：...`，完成 1:1 互斥锁定；
  5. **第五步【极速智能上架】**：直接发送 Ozon SKU 列表或 `.txt` 文件，全自动全要素搬家上架！
...
### 0. 🤖 Antigravity 智能体技能标准 5 步上架 SOP (推荐)

如果您在 Google Antigravity / Claude Code / Codex 等 AI 对话环境中使用本技能，安装本技能后，只需按以下 5 步即可完成专属授权与全自动上架：

```text
第1步【获取会话ID与支付】：在新聊天窗口发送「获取会话ID」，获取当前窗口专属 Conversation ID，并在收银台支付宝扫码支付 600 元（¥600/店铺，一店一码 1:1 独立隔离）。
第2步【自动获取授权码】：支付成功后系统秒级自动签发专属防伪授权码 (License Key)。
第3步【激活当前窗口】：在当前窗口发送「激活授权 <你的商业授权码>」。
第4步【绑定目标店铺】：发送「绑定店铺 店铺简称：... API令牌：... 仓库ID：...」，完成 1:1 互斥锁定。
第5步【极速智能上架】：直接发送 Ozon SKU 列表或 .txt 文本路径，全自动全要素搬家上架！
```
