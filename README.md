...
- **简洁商品结果卡片**：向用户发送 SKU、商品名称、当前状态及可用的 WB 商品链接；未经平台核实的在售状态标为“待确认”。

### 9. 🔐 会话级商业授权门禁、单窗口 1:1 店铺互斥与代理商线下专属授权闭环 (Mode B 商业版)
- **极简 3 步线下授权与开通流转**：
  1. **提取窗口专属 ID**：用户直接发送 `获取授权码`、`wb上架激活码` 或 `会话ID`，系统**自动秒级捕获当前窗口专属 Conversation ID** 并输出标准 3 步线下开通指引面板；
  2. **联系代理商开通**：用户将该窗口专属 ID 发送给专属代理商/客服人员，线下完成授权结算并获取专属店铺授权码；
  3. **输入指令一键激活**：收到代理商发来的授权码后，在聊天框直接发送 `激活授权 <授权码>` 即可立即激活本窗口；
  4. **绑定店铺与极速搬家**：激活完成后，系统主动引导提示用户输入：`绑定店铺 店铺简称：... API令牌：... 仓库ID：...` 完成 1:1 互斥锁定，随后提供 Ozon SKU 开启全自动全要素极速搬家上架！
- **基于 Conversation ID 的窗口级隔离（无需绑定物理机器码）**：系统以 Antigravity 聊天会话窗口为物理边界，无需客户提取繁琐的硬件指纹，跨设备、换电脑无缝使用；窗口 1 绑定店铺 A，窗口 2 绑定店铺 B，彻底杜绝商品串店误传。
- **多代理商专属代号绑定与自动分润结算体系**：
  - 支持在窗口输入：`代理商 <代号>`、`渠道 <代号>` 或 `渠道码 <代号>`（例如：`代理商 zhuang_qz` 或合作代理商代号），系统自动将当前窗口会话与该代理商永久绑定；
  - 针对泉州庄总（`zhuang_qz`），面板严格引导联系庄总线下结算发码；针对其他代理商，面板动态匹配其代号并自动将所有订单与店铺授权归集结算至其名下；
  - 支持在发行包 `config.json` 中预设 `{"agent_id": "..."}`，终端客户解压即用，100% 自动锁定代理商归属。
- **RSA-2048 非对称防伪签名**：管理员私钥签名，客户端内置公钥验签，彻底杜绝离线伪造授权。
- **零信任商业授权门禁**：新创建的 Antigravity 聊天窗口默认处于未授权状态 (`UNAUTHORIZED`)，未激活有效授权码强行阻断上架。
- **单窗口 1:1 店铺互斥锁定 (Store Mutex Lock)**：每一个对话窗口严格限制仅允许绑定一家 Wildberries 店铺，严禁混绑并发。
- **平滑安全更换店铺与单窗口最多换店 1 次限制**：支持通过 `切换店铺` 指令更换绑定的 WB 店铺。为杜绝客户买 1 个授权轮播换绑多家店铺白嫖并彻底防止商品串店，每个商业窗口最多仅允许更换 1 次店铺（换店后永久锁定），超级管理员 (Master VIP) 不受此限。
- **2 天全功能免费试用授权机制 (2-Day Free Trial)**：
  - 降低新客户决策门槛，支持 2 天（48 小时）全功能免费试用体验，1 窗口 1 店铺独立隔离；
  - 用户在聊天框发送 `申请试用` / `试用授权` / `试用码`，系统秒级自动签发 2 天全功能试用授权码并解锁当前窗口；
  - 代理商/管理员可随时运行 `python scripts/session_manager.py generate-trial --name "试用商户"` 批量为商户生成 2 天试用码；
  - 试用期内享受 100% 完整极速上架能力，试用满意后输入正式授权码即可无缝升级，历史店铺绑定 100% 继承。
...
### 0. 🤖 Antigravity 智能体技能标准极速开通 SOP (推荐)

如果您在 Google Antigravity / Claude Code / Codex 等 AI 对话环境中使用本技能，安装本技能后，只需以下极简流转即可完成专属授权与全自动上架：

```text
第1步【获取窗口专属 ID】：在聊天窗口直接发送「获取授权码」或「wb上架激活码」，系统秒级自动提取当前窗口专属 Conversation ID。
第2步【联系代理商开通】：将该 ID 发送给您的专属代理商/客服人员，线下完成授权结算并获取专属店铺授权码。
第3步【输入指令激活】：收到代理商发来的授权码后，在聊天框直接发送指令「激活授权 <代理商提供的授权码>」立即激活当前窗口。
第4步【主动提示绑定店铺与上架】：店铺激活成功后，系统主动提示您输入绑定店铺并提供 Ozon SKU，全自动全要素搬家上架！
```

> 💡 **单窗口 1:1 店铺隔离说明**：每个聊天窗口独立绑定一家 Wildberries 店铺。新开窗口可绑定新店铺，无需绑定硬件机器码，换电脑换设备无缝使用！
...
售价6倍，库存5
```
机器人会处理商品上架任务，并向用户回传简洁的 SKU、状态与 WB 商品链接。平台在售状态未经核实时，卡片会标记为“待确认”。
