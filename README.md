...
### 9. 🔐 会话级商业授权门禁、单窗口 1:1 店铺互斥与极简智能开通闭环 (Mode B 商业版)
- **极简全自动智能流转闭环**：
  1. **获取授权与支付**：用户直接发送 `获取授权码` 或 `wb上架激活码`，系统**自动捕获当前窗口专属 Conversation ID** 并直接分发专属支付宝支付链接与 100x100 高清二维码（¥600/店铺/年，自支付当日起 365 天有效，一店一码 1:1 独立互斥隔离）；
  2. **自动下发与自动激活**：支付宝扫码支付成功后，系统秒级自动签发防伪授权码并**自动激活当前会话窗口**（无需手动输入！）；
  3. **主动提示绑定店铺**：激活完成后，系统主动引导提示用户输入：`绑定店铺 店铺简称：... API令牌：... 仓库ID：...`，完成 1:1 互斥锁定；
  4. **主动提示提供 SKU 并极速搬家**：店铺绑定成功后，系统主动提示用户提供 Ozon SKU，用户直接发送 SKU 列表或 `.txt` 文件，全自动全要素搬家上架！
...
```text
第1步【获取授权与支付】：在聊天窗口直接发送「获取授权码」或「wb上架激活码」，系统自动获取当前窗口专属 Conversation ID 并分发支付宝支付 100x100 二维码（¥600/店铺/年，自支付当日起 365 天有效，一店一码 1:1 独立隔离）。
第2步【自动下发与激活】：支付宝扫码支付成功后，系统秒级自动签发专属防伪授权码并【自动激活当前窗口】。
第3步【主动提示绑定店铺】：激活成功后，系统主动提示您输入：「绑定店铺 店铺简称：... API令牌：... 仓库ID：...」，完成 1:1 互斥锁定。
第4步【主动提示提供SKU并上架】：店铺绑定成功后，系统主动提示您提供 Ozon SKU，直接发送 SKU 列表或 .txt 文本，全自动全要素搬家上架！
```
