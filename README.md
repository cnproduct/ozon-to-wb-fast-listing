...
### 9. 🔐 会话级商业授权门禁与 1:1 店铺隔离系统 (Mode B: License Gatekeeper & Store Mutex)
- **零信任商业授权门禁**：新创建的 Antigravity 聊天窗口默认处于未授权状态 (`UNAUTHORIZED`)，必须通过授权码激活 (`激活授权 <License_Key>`) 方可调用上架引擎，杜绝未授权使用。
...
- **统一会话管理套件 (`scripts/session_manager.py`)**：支持管理员签发多租户/到期时间/配额授权码，会话档案跨重启全局持久化。
