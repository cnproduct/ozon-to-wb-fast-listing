...
- **1 天全功能免费试用授权机制 (1-Day Free Trial)**：
  - 降低新客户决策门槛，支持 1 天（24 小时）全功能免费试用体验，1 窗口 1 店铺独立隔离；
  - 用户在聊天框发送 `申请试用` / `试用授权` / `试用码`，系统秒级自动签发 1 天全功能试用授权码并解锁当前窗口；
  - 代理商/管理员可随时运行 `python scripts/session_manager.py generate-trial --name "试用商户"` 批量为商户生成 1 天试用码；
