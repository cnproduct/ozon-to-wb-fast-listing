...
- **极简全自动智能流转闭环**：
  1. **获取授权与支付**：用户直接发送 `获取授权码`，系统**自动捕获当前窗口专属 Conversation ID** 并直接分发专属支付宝支付链接与二维码（¥600/店铺/年，自支付当日起 365 天有效，一店一码 1:1 独立互斥隔离）；
...
```text
第1步【获取授权与支付】：在聊天窗口直接发送「获取授权码」，系统自动获取当前窗口专属 Conversation ID 并分发支付宝支付二维码（¥600/店铺/年，自支付当日起 365 天有效，一店一码 1:1 独立隔离）。
