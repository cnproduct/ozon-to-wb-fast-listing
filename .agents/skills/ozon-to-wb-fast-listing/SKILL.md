...
“全要素上架质量与合规标准审核报告”中的覆盖率、条码、定价公式、包装推导、品牌处理、相册数量、归档字段和异步等待时间，仅留内部核验；未经独立复核，不称作在售或自动生效。每次相关对话结束前，有可复用的新问题或平台、政策、商品、物流、用户操作变化时，按仓库 `references/learning-loop.md` 用 `scripts/record_learning.py` 保存去标识化的本地候选。只在官方/实际证据核验、去重后通过草稿 PR 更新正式 Skill；无新增不记。

客户开始上架或进度消息只说“智能体将为您全自动上架”或“正在为您上架”，可附店铺和商品数量；不罗列内部执行步骤。售价、库存等需客户确认的业务参数可简明列出。
...
> -1. **【客户店铺与 API Token 安全隔离铁律（多客户适配）】**：
>     - **禁止跨客户混用**：每个客户拥有完全独立的 Wildberries 店铺。AI 在执行上架前，必须确认当前环境所绑定的 API Token 归属于该客户本人。
>     - **智能动态探测**：系统优先从 `config.json`、系统环境变量 `WB_API_TOKEN` 或命令行参数获取客户专属密钥。若首次使用或未检测到有效 Token，AI 必须主动且礼貌地提示客户提供其店铺的 WB API Token 与仓库 ID（并指引客户参考《使用说明书》第 2.1 节快速申请），严禁未经授权操作他人店铺！
...
> 0.1. **【Ozon 绿标 5 倍定价与 5 折活动铁律】**：
>    - **基准价格与倍数铁律**：所有款式统一以 Ozon 绿标特惠价为基准（以 SKU 5289673242 为锚点：绿标 144.28，严禁误填为俄罗斯买家端卢布原值 1811₽），**一律按 5 倍乘数计算 Wildberries 售价**（例：$144.28 \times 5 = 721.4$）。
>    - **5 折大促活动锁定**：所有商品均参与平台 5 折活动，**乘 5 得到的即为打 5 折后的最终实售价**。
>    - **划线标价反推**：划线标价 = 实售价 $\times 2$（例：$721.4 \times 2 = 1443$）。
>    - **WB 价格接口上传规范**：`price` 填划线标价（如 1443），`discount` 填 50，买家端实付精准为 5 折后售价（721.4）。
> 0.2. **【汇报与呈报规范（严禁绿标虚高与公式脱节）】**：
>    - 表格与汇报中列出的【Ozon 绿标价格】必须为基准锚点值（如 144.28、188.66），**严禁直接填入 1811₽、2368₽ 等卢布买家原值**，避免用户产生“绿标怎么这么贵”的错觉与视觉断层。
>    - 仅在上架前确认定价策略或用户明确索要价格核算时，列出完整对齐链路；上架完成后的客户结果遵守本文件顶部的简洁汇报规则：`Ozon 绿标价格` ➔ `Wildberries 5折实售价`（绿标 $\times 5$）➔ `Wildberries 划线标价`（50% 折扣）。
>    - 示例格式：`SKU 5289673242: Ozon 绿标价: 144.28 ➔ WB 5折实售: 721.4 (划线标价: 1443, 50% 折扣)`。
> 0.3. **【后台实时全量数据穿透核验准则】**：
...
### 22. Ozon 绿标基准价与买家端卢布原值严格隔离规范 (Ozon Green Base Price vs Buyer Ruble Standard)

1. **核心痛点与问题根源**：
   - 从 Ozon 俄罗斯前台页面直接通过爬虫或正则表达式提取到的数值是买家端前台卢布标价（`cardPrice` 或 `price`，例如 `1811 ₽`、`2368 ₽`）。
   - 用户系统与商业决策中认定的【Ozon 绿标价格】是以基准计价的锚定值（例如 SKU `5289673242` 的绿标价格是 **`144.28`**，换算关系为 $1811 \div 12.551982 = 144.28$）。
   - 若在交付给用户的表格与报表中直接将 `1811` 标在“Ozon 绿标价”列，会导致用户产生“绿标怎么这么贵？（上千卢布）”的极大困惑，且与后文“绿标 × 5 = 721.4”的数学逻辑产生脱节断层。

2. **标准化工程规范**：
   - **变量与字段命名严格隔离**：
     - `ozon_buyer_rub`：专用于存储 Ozon 俄罗斯前台买家端原始卢布数值（如 `1811.0`）；
     - `ozon_green_price`：专用于存储用户认定的基准绿标底价（如 `144.28`，公式：$\text{round}(P_{\text{rub}} / 12.551982, 2)$）。
   - **用户端呈报与表格格式闭环**：
     - 凡是面向用户的表格展示，【Ozon 绿标价】列必须填入 `ozon_green_price`（如 144.28）；
     - 紧邻的【WB 5折实售目标】列必须展示 $144.28 \times 5 = \mathbf{721.4}$；
     - 划线标价展示 $721.4 \times 2 = \mathbf{1443}$（50% 折扣活动）。
     - 保证用户一眼看去，公式计算与数值闭环清晰无二义性。
