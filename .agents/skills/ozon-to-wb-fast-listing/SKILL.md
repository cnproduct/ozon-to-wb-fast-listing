...
用户输入「申请试用」时，获取当前真实 Conversation ID，执行 `python scripts/session_manager.py trial --conversation-id <当前真实会话ID>`。首次领取签发恰好 48 小时的授权；重复申请返回原试用状态，不重置时间；已有正式授权保留。仅在命令返回成功后报告试用已开通。若无法获得真实会话 ID 或签发失败，应说明原因。旧版技能必须更新后才会读取这条规则。 试用由云端网关签发且使用时在线验真，客户端不能自行生成签名私钥。

同一公网 IP 本轮最多领取 3 个不同会话窗口。第 4 个窗口被拒绝时，原样提示：避免线路拥堵，您只允许3个试用窗口。如需使用更多窗口，请跟代理商申请付费授权窗口。不要自行绕过云端配额。
