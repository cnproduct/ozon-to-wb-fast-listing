    ...
    def parse_command(self, raw_text: str, conversation_id: Optional[str] = None) -> Optional[str]:
        ...
        if text.lower() in [k.lower() for k in trigger_keywords] or (
            ...
            return (
                ...
                f"2️⃣ **第2步【自动下发与激活】**：支付成功后系统秒级自动签发专属防伪授权码并【自动激活当前窗口】（无需手动输入！）；\n"
                f"3️⃣ **第3步【主动提示绑定店铺】**：激活完成后系统主动提示您输入：`绑定店铺 店铺简称：... API令牌：... 仓库ID：...`，完成 1:1 互斥锁定；\n"
                f"4️⃣ **第4步【主动提示提供SKU并上架】**：店铺绑定成功后，直接发送 Ozon SKU 列表或 .txt 文档，全自动全要素搬家上架！\n"
            )
