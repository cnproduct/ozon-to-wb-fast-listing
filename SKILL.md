...
> 0.1. **【跨境店铺人民币统一价格核算法则（CNY 6倍实售 / 12倍划线 / 50%官方大促与整型约束）】**：
...
>      - 划线标价整型下发：$P_{\text{strike\_int}} = \text{round}(P_{\text{ozon\_CNY}} \times 12)$（如 $68.13 \times 12 = 817.56 \to \mathbf{818 \text{ 元}}$）
>      - 后台实售折后价：$P_{\text{sell\_int}} = \text{round}(P_{\text{strike\_int}} \times (1 - 50\%)) = \mathbf{409 \text{ 元}}$（精准符合 408.78 元 6 倍目标，买家付款后以此金额向卖家全额结算）
>    - **买家端前台呈现与平台补贴**：
...
> 0.2. **【汇报与呈报规范】**：
>    - 每次汇报明确列出完整对齐链路：`Ozon 绿标人民币价 (CNY)` ➔ `WB 5折实售价`（绿标 $\times 6$）➔ `WB 划线标价`（绿标 $\times 12$，50% 折扣）➔ `买家前台卢布折算与补贴说明`。
>    - 示例格式：`SKU 5363583814: Ozon 绿标价: 68.13 元 ➔ WB 5折实售: 409 元 (划线标价: 818 元, 50% 折扣) | nmID: 1583834747`。
> 0.3. **【后台实时全量数据穿透核验准则】**：
...
### 22. 跨境店铺人民币统一价格核算法则与模型 (Cross-Border CNY 6x Sell & 12x Strike Pricing Model)
...
1. **核心原则与业务底座**：
   ...
   - **公式逻辑闭环**：
     - **卖家后台实售目标价（买家到手折后价，CNY）**：
       $$P_{\text{sell\_CNY}} = P_{\text{ozon\_CNY}} \times 6$$
       *(例如 $68.13 \times 6 = 408.78 \text{ 元}$)*
     - **卖家后台划线标价（原价，CNY）**：
       $$P_{\text{strike\_CNY}} = P_{\text{sell\_CNY}} \times 2 = P_{\text{ozon\_CNY}} \times 12$$
       *(例如 $408.78 \times 2 = 817.56 \text{ 元}$)*
     - **官方促销折扣率**：
       $$\text{Discount} = 50\%$$
   - **平台 API 整数化强制约束 (Whole Number Rule)**：
     - Wildberries API 严格拒绝浮点数价格（返回 `Price should be a whole number`），必须执行整型四舍五入：
       $$P_{\text{strike\_int}} = \text{round}(P_{\text{strike\_CNY}}) = \text{round}(817.56) = \mathbf{818 \text{ 元}}$$
       $$P_{\text{sell\_int}} = \text{round}(P_{\text{strike\_int}} \times 0.5) = \mathbf{409 \text{ 元}}$$
       *(后台实际按 409 元向卖家结算，与 408.78 元仅差 0.22 元整型舍入误差)*
...
2. **买家端前台卢布展示与平台补贴机制**：
   ...
   - **平台忠诚度补贴（SPP）**：若平台对买家开放了补贴（SPP），买家前台最终支付金额会比汇率折算价更低，补贴差额由 WB 平台承担，卖家依然按设定的人民币实售价（409 元）全额回款。

3. **呈报与核验格式规范**：
   - 凡是面向用户的核验表格，清晰展示人民币基准及全链路对齐数据：
     - 【Ozon 绿标人民币价 (CNY)】：68.13 元
     - 【WB 5折实售价 (CNY)】：409 元（理论 408.78 元）
     - 【WB 划线标价 (CNY)】：818 元（理论 817.56 元）
     - 【活动折扣】：`50%`
