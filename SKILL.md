...
> 0. **【用户给 SKU 或 txt 文档后的第一响应铁律（问完再去工作）】**：
...
>    - **先问后干绝对红线**：解析到有效 SKU 列表后，向用户呈报已成功识别的 SKU 数量与明细，**严禁在未获得用户价格与库存指示前擅自开始任何耗时的后台抓取或建卡工作！AI 的第一步动作必须立即暂停并直接向用户询问【上架的售价策略】与【库存设置】**（附带默认推荐标准大促方案：**最终实售价设为 Ozon 原价的 5 倍**，上架划线原价标为 10 倍，配置 **50% 官方大促折扣**，例如 Ozon 50₽ ➔ WB 原价标 500₽，打 5 折促销到手 250₽，前台展示高吸引力打折标，买家实付精准为 5 倍；莫斯科1仓 200 件现货）。**必须在收到用户的明确回复与指示后，方可启动后续的数据抓取、建卡、多图直传、价格下发与库存注入全流程！**
