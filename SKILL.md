...
- **长宽高顺序绝对不颠倒**：严格读取 Ozon 原文参数顺序（`Длина х Ширина х Высота`），严禁凭主观猜测互换长和宽（例如猫砂铲长13cm、宽26cm、高6cm，绝不能误设为长26宽13）。
- **毛重精确支持浮点数 KG**：`weightBrutto = round(weight_g / 1000.0, 2)`（如 1.5kg, 0.18kg, 0.25kg, 7.0kg 等），原生浮点直传，严禁整型截断。
- **必须联动抓取 Ozon `/features/` 完整特征页**：Ozon 主页（`/product/{sku}/`）默认折叠了大量底层参数，必须同步请求 `/product/{sku}/features/`，将折叠在深处的毫米级裸机尺寸（`Размеры, мм`）、净重（`Вес товара, г`）、线长（`Длина шнура, м`）等全部 100% 提取，绝不遗漏。
