...
> 0.1. **【纯卢布绿标价 6 倍实售与 2 倍划线标价法则（绝不涉及人民币换算）】**：
...
> 0.2. **【汇报与呈报规范】**：
>    - 每次汇报明确列出完整对齐链路：`Ozon 绿标价格 (RUB)` ➔ `Wildberries 5折实售价`（绿标 $\times 6$）➔ `Wildberries 划线标价`（绿标 $\times 12$，50% 折扣）。
>    - 示例格式：`SKU 2688566845: Ozon 绿标价: 4,059 ₽ ➔ WB 5折实售: 24,354 ₽ (划线标价: 48,708 ₽, 50% 折扣)`。
> 0.3. **【后台实时全量数据穿透核验准则】**：
...
### 22. Ozon 纯卢布绿标价 6 倍实售与 2 倍划线标价法则 (Ozon Direct RUB Green Badge Pricing Model)

1. **核心原则与绝对红线**：
   - **绝不涉及任何人民币或汇率换算**：Wildberries 价格接口直接下发卢布数值，严禁任何除以汇率（如 11.67、12.55 等）的操作！
   - **精准锚定 Ozon 绿标卡价 (`ozon_green_price`)**：基准价格源自 Ozon 前台绿框标注的 Ozon 银行卡专享特惠价（若无绿标特惠则取当前促销实售价）。
   - **公式逻辑闭环**：
     - 买家到手实售价 (RUB)：$P_{\text{sell}} = \text{round}(P_{\text{ozon\_green}} \times 6)$（或按用户指定的倍数）
     - 官方划线标价 (RUB)：$P_{\text{strike}} = \text{round}(P_{\text{sell}} \times 2) = \text{round}(P_{\text{ozon\_green}} \times 12)$
     - 官方促销折扣率：$50\%$
     - 买家端实付金额：$P_{\text{strike}} \times (1 - 0.5) = P_{\text{sell}}$（精准等于 Ozon 绿标价的 6 倍）

2. **呈报与表格格式规范**：
   - 凡是面向用户的核验表格，清晰展示各阶段真实卢布数值：
     - 【Ozon 绿标价 (RUB)】：直接填入爬取到的 Ozon 绿标卢布价（如 4,059 ₽）；
     - 【WB 5折实售价 (RUB)】：展示绿标价 × 6（如 24,354 ₽）；
     - 【WB 划线标价 (RUB)】：展示实售价 × 2（如 48,708 ₽）；
     - 【活动折扣】：标注 `50%`。
   - 保证用户一眼看去，公式严谨、买家实付价格与商业预期 100% 对齐。
