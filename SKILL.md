...
> 0.1. **【定价与上传纯卢布铁律（绝对严禁涉及任何人民币）】**：
>    - **纯卢布绝对红线**：所有价格计算、API 上传价格与内部数据流，**一律且仅使用卢布（RUB）！绝对严禁进行任何人民币（CNY/元）折算，严禁使用汇率除算！不要管人民币价格！**
>    - **统一极简公式**：
>      - Wildberries 实售价：$P_{\text{wb\_sell}} = P_{\text{ozon\_green}} \times \text{multiplier}$（例：$2,368 \times 6 = 14,208$ ₽）
>      - Wildberries 划线标价（50%折扣）：$P_{\text{wb\_strike}} = P_{\text{wb\_sell}} \times 2$（例：$14,208 \times 2 = 28,416$ ₽）
>      - 向 WB 价格接口上传：`price` 填 $P_{\text{wb\_strike}}$，`discount` 填 50%，前台实售即为 $P_{\text{wb\_sell}}$。
> 0.2. **【回复与汇报格式铁律（仅汇报卢布，严禁出现人民币）】**：
>    - **纯净呈报规范**：每次向用户呈报商品明细或回复进度时，**严禁出现任何“人民币”、“元”、“CNY”字样**。仅明确列出每款商品的：
>      - `Ozon Green Price`（Ozon 绿标售价，₽）
>      - `Wildberries Price`（Wildberries 售价，₽，以及 50% 折扣划线标价 ₽）
>    - 示例格式：`SKU 1914097578: Ozon Green Price: 2,368 ₽ ➔ Wildberries Price: 14,208 ₽ (标价: 28,416 ₽, 50% 折扣)`。
