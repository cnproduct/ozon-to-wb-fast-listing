...
>
> -1. **【零信任商业授权门禁、自动分发激活与单窗口 1:1 店铺互斥隔离铁律 (Mode B 商业授权)】**：
>     - **商业收费法则与单店一码制**：**每家 Wildberries 店铺收费 600 元人民币（¥600/店铺/年，自支付当日起 365 天有效）**，每个店铺对应一个专属防伪授权码（1店1码）。
>     - **零信任商业授权门禁与全自动极简流转闭环**：每一个新开启的 Antigravity 聊天会话窗口默认处于未授权状态（`UNAUTHORIZED`）。任何新窗口在执行上架或调用搬家引擎前，系统必须核验当前窗口会话 ID 的授权状态。
>     - **全自动智能开通与极速上架 SOP**：
>       1. **获取授权与支付**：用户直接发送 `获取授权码` 或 `wb上架激活码`（或未授权时任意输入），智能体**自动获取当前窗口 Conversation ID** 并直接分发专属支付宝支付链接与 150x150 高清二维码（¥600/店铺/年，365天有效，一店一码 1:1 独立隔离）；
>       2. **自动下发与自动激活**：支付宝扫码支付成功后，系统秒级自动签发授权码并**自动激活当前窗口**（无需手动输入！）；
>       3. **主动提示绑定店铺**：激活成功后，系统主动提示用户绑定目标店铺（发送：`绑定店铺 店铺简称：... API令牌：... 仓库ID：...`），完成 1:1 互斥锁定；
>       4. **主动提示提供 SKU 并极速上架**：店铺绑定成功后，系统主动提示用户提供 Ozon SKU，用户直接发送 SKU 列表或 `.txt` 文件，全自动全要素极速搬家上架！
...
>     - **Cloudflare Workers 云端鉴权网关、远程在线封禁与用量看板 (Stage 2 商业化扩展)**：接入轻量 Serverless 网关（`cloud/worker.js`）。支持毫秒级远程封禁违规授权码、充值续期与实时用量看板（Web `/admin`），并在离线或弱网时自动平滑回退至本地 RSA-2048 验签，兼顾管理控制力与商户使用稳定性。
