...
>     - **2 天全功能免费试用授权机制 (2-Day Full-Feature Free Trial)**：
>       - **48 小时全功能体验**：为降低客户决策门槛，系统支持 2 天（48 小时）免费全功能试用授权，每店限试用 1 次，享受 100% 完整极速搬家、买家端参数饱和注入、多图直传、现货库存及 50% 官方大促折扣价格闭环；
>       - **自助与一键开通**：用户在聊天窗口直接发送 `申请试用`、`试用授权`、`试用码` 或 `2天免费试用`，系统即可自动为当前窗口生成并激活 2 天试用授权；
>       - **代理商/管理员快速签发**：代理商或管理员可随时执行 `python scripts/session_manager.py generate-trial --name "试用客户"` 生成 2 天试用授权码发放给客户；
