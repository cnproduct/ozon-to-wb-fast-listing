...
>    - **基准价格与倍数铁律**：所有款式统一以 Ozon 绿标特惠价为基准（以 SKU 5289673242 为锚点：绿标 144.28），**一律按 5 倍乘数计算 Wildberries 售价**（例：$144.28 \times 5 = 721.4$）。
...
> 0.2. **【汇报与呈报规范】**：
>    - 每次汇报明确列出：`Ozon 绿标价格` ➔ `Wildberries 5折实售价`（绿标 $\times 5$）➔ `Wildberries 划线标价`（50% 折扣）。
