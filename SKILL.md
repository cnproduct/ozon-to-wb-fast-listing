...
>     - **零信任商业授权门禁**：每一个新开启的 Antigravity 聊天会话窗口默认处于未授权状态（`UNAUTHORIZED`）。任何新窗口在执行上架或调用搬家引擎前，系统必须核验当前窗口会话 ID 的授权状态。若未激活，系统立即阻断上架并主动提示标准 5 步开启指引。
>     - **全流程标准 5 步极速开通与上架 SOP**：
...
>       3. **第三步【激活当前窗口】**：在当前窗口对话框发送 `激活授权 <你的商业授权码>`；
