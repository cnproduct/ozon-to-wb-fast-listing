...
1. **【先问后干】第一响应铁律**：
   - 收到用户提供的 SKU（直接粘贴或 `.txt` 文档）后，第一步必须立即向用户呈报已识别的有效数字 SKU 列表，并询问上架的【售价策略（实售倍数 M）】与【库存设置】（提供默认推荐方案：Ozon 绿标价 6 倍实售，划线价 12 倍，50% 官方大促折扣，莫斯科1仓现货库存 5 件；或根据用户最新指定倍数如 3 倍、5 倍及指定库存执行）。
   - **允许并响应动态倍数变更**：倍数并非固定只能是 6 倍，系统全力支持用户灵活变更为 3 倍、5 倍、6 倍、8 倍等任意倍数；若用户在指令中已指定倍数（如“按 3 倍上架”），系统直接执行，严禁多余打扰。
   - 严禁在用户未确认前擅自开始耗时的抓取或建卡工作。

2. **【跨境店铺人民币统一价格核算法则（动态倍数 M 实售 / 2M 划线标价 / 50%官方大促与整型约束）】**：
