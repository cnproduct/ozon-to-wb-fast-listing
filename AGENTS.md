...
1. **【零信任商业授权门禁、会话ID绑定与单窗口 1:1 店铺互斥隔离铁律 (Mode B 商业授权)】**：
   - **商业收费法则与单店一码制**：**每家 Wildberries 店铺收费 600 元人民币（¥600/店铺，按店铺核算）**，每个店铺对应一个专属不可篡改授权码（1店1码）。
   - **零信任商业授权门禁与标准 6 步 SOP**：每一个新开启的 Antigravity 聊天会话窗口默认处于未授权状态（`UNAUTHORIZED`），系统主动引导用户按标准 6 步极速开通：
